...
int do_acl_access()
{
    ...
    if((xchange !=0) || (ychange != 0))
    {
    	...
    	new_x = (new_x>MAZE_X_SIZE_M1) ? MAZE_X_SIZE_M1 : (new_x<0) ? 0 : new_x;
    	new_y = (new_y>MAZE_Y_SIZE_M1) ? MAZE_Y_SIZE_M1 : (new_y<0) ? 0 : new_y;
        ...
        switch(maze_arr[new_y][new_x])
        {
        	...
        	case STAR:
        	{
    			...
    			{
    				if((GobStarsCache.x[i] == new_x) && (GobStarsCache.y[i] == new_y))
    					isAvl = 1;
    			}
    			...
    			if(isAvl == 0)
    			{
					...
					GobStarsCache.x[arr_index] = new_x;
					GobStarsCache.y[arr_index] = new_y;
					GobStarsCache.index++;
    			}
